defmodule NYSETLWeb.CommcareCasesControllerTest do
  ...
  describe "/commcare_cases" do
    test "GET checks endpoint", %{conn: conn} do
      conn = get(conn, Routes.commcare_cases_path(@endpoint, :create_or_update))
      assert response(conn, 200) =~ "OK"
    end

    test "create an oban job w/ commcare_case_id and domain and no action arg", %{
      conn: conn,
      ...
      conn = post(conn, Routes.commcare_cases_path(@endpoint, :create_or_update), patient_case)
    ...
    test "401 when credentials are wrong", %{conn: conn, midsomer_patient_case: patient_case} do
      conn =
        ...
        |> post(Routes.commcare_cases_path(@endpoint, :create_or_update), patient_case)
  ...
  describe "/commcare_cases/create" do
    test "GET checks endpoint", %{conn: conn} do
      conn = get(conn, Routes.commcare_cases_path(@endpoint, :create))
      assert response(conn, 200) =~ "OK"
    end

    test "create a 'create' oban job w/ case_id and domain", %{
      conn: conn,
      midsomer_county: midsomer,
      midsomer_patient_case: patient_case
    } do
      ...
      assert response(conn, 202) =~ "Accepted"
      assert_enqueued(worker: CaseImporter, args: %{action: "create", commcare_case_id: Map.fetch!(patient_case, "case_id"), domain: midsomer.domain})
      assert {:error, :not_found} = Commcare.get_index_case(case_id: patient_case["case_id"], county_id: midsomer.fips)
    end
  ...
  describe "/commcare_cases/update" do
    test "GET checks endpoint", %{conn: conn} do
      conn = get(conn, Routes.commcare_cases_path(@endpoint, :update))
      assert response(conn, 200) =~ "OK"
    end

    test "create an 'update' oban job w/ all params", %{
      conn: conn,
      midsomer_county: midsomer,
      midsomer_patient_case: patient_case
    } do
      ...
      conn = post(conn, Routes.commcare_cases_path(@endpoint, :update), patient_case)

      assert response(conn, 202) =~ "Accepted"
      assert_enqueued(worker: CaseImporter, args: %{action: "update", commcare_case: patient_case})
      assert {:error, :not_found} = Commcare.get_index_case(case_id: patient_case["case_id"], county_id: midsomer.fips)
    end
    ...
    test "401 when credentials are wrong", %{conn: conn, midsomer_patient_case: patient_case} do
      conn =
        conn
        |> put_commcare_auth("bad-password")
        |> post(Routes.commcare_cases_path(@endpoint, :update), patient_case)
