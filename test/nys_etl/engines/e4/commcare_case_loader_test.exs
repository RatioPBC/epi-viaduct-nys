defmodule NYSETL.Engines.E4.CommcareCaseLoaderTest do
  ...
  describe "perform" do
    ...
    |> test do
      ...
      receive do
        {:xml, xml} ->
          %Commcare.IndexCase{events: [update_event]} = Repo.preload(index_case, :events)
      ...
      {:ok, reloaded_index_case} = Commcare.get_index_case(case_id: case_id, county_id: county.id)
      assert reloaded_index_case.data["full_name"] == "Original Full Name"

      reloaded_index_case |> assert_events(["send_to_commcare_succeeded"])
    end
