defmodule NYSETL.Engines.E5.Processor do
  ...
  * case_id does not exist, Person cannot be matched:
  ...
  IndexCase records are only created for Person records that have already been identified from ECLRS data. This
  decision was made so that NYS Viaduct would store and retain less PHI — this was decided by the Viaduct team, and
  was *NOT* based on any expressed business needs.
  """
  ...
  def process(case: case, county: county) do
    with {:error, :not_found} <- find_and_update_index_case(case, county),
         {_case_id, patient_key, dob, lab_results} <- extract_case_data(case),
         {:ok, person, finder} <- find_person(case, dob, patient_key) do
      {:ok, index_case} = create_index_case(case, person, county)
      ...
      {:error, :not_found, :person} ->
        cond do
          case_is_a_stub?(case) ->
            :telemetry.execute([:extractor, :commcare, :index_case, :stub_found], %{count: 1})

          case_has_lab_result?(case) ->
            :telemetry.execute([:extractor, :commcare, :index_case, :with_lab_result, :person_not_found], %{count: 1})

          true ->
            :telemetry.execute([:extractor, :commcare, :index_case, :without_lab_result, :person_not_found], %{count: 1})
        end

        {:error, case, :no_person}
    end
  end

  def extract_case_data(case) do
    case_id = case["case_id"]
    lab_results = lab_results(case)
    patient_key = find_patient_key(case, lab_results)
    dob = parse_dob(case)
  ...
  def find_and_update_index_case(case, county) do
    Commcare.get_index_case(case_id: case["case_id"], county_id: county.fips)
    |> case do
      {:error, :not_found} ->
        {:error, :not_found}

      {:ok, index_case} ->
        Commcare.update_index_case_from_commcare_data(index_case, case["properties"])
        |> case do
          {:ok, ^index_case} ->
            Logger.info(
  ...
  def find_person(case, dob, patient_key) do
    Logger.debug("[#{__MODULE__}] trying to find a person matching index_case case_id=#{case.case_id}")

    find_person(patient_key: patient_key) ||
      find_person(case, dob: dob) ||
      {:error, :not_found, :person}
  end
